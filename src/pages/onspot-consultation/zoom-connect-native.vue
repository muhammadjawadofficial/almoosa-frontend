      ...
      <div
        ...
        :style="
          isParticipantJoined && false
            ? '--profile-image: url(' +
              getImageUrl(getOnspotConsultation.user) +
              ')'
            : ''
        "
      >
        ...
        <div class="display-name responsive">
          {{
            isParticipantJoined
              ? getFullName(getOnspotConsultation.user)
              : $t("noOneJoined")
          }}
        </div>
      </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      let sessionName = this.getOnspotConsultation.connect.sessionName;
      let token = this.getOnspotConsultation.connect.signature;
      let username = this.getFullName(this.getUserInfo);
      let password = this.getOnspotConsultation.connect.password;
    ...
    async ringPatient() {
      ...
      try {
        ...
        let response = await appointmentService.ringPatient({
          session_name: this.getOnspotConsultation.connect.sessionName,
        });
